    ...
    dotenv_path.write_text(
        "\n".join(
            [
                ...
                "ACT_AI_SELF_REPAIR_ENABLED=true",
                "INVALID LINE",
            ]
        ),
        encoding="utf-8",
...
def test_load_runner_local_env_overrides_shell_for_openai_keys(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("ACT_AI_SELF_REPAIR_ENABLED", "false")
    ...
    assert os.getenv("ACT_AI_SELF_REPAIR_ENABLED") == "true"
    assert os.getenv("ACT_CAPTURE_STEPS") == "true"
